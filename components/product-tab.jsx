...
export default function ProductTabs({ product }) {
  ...
  return (
    <div className="mb-12">
      {/* Tab Navigation */}
      <div className="flex gap-6 mb-6 border-b-2 overflow-x-auto" style={{ borderColor: "#f0f0f0" }}>
        {["overview", "specifications", "reviews"].map((tab) => (
          <button
            ...
            style={{ color: activeTab === tab ? "var(--primary-dark)" : "#999" }}
          >
            {tab}
            {activeTab === tab && (
              <span
            ...
            <ul className="grid grid-cols-1 md:grid-cols-2 gap-3">
              {product.features.map((feature, idx) => (
                <li key={idx} className="flex items-start gap-3 text-sm">
                  <span className="text-lg flex-shrink-0" style={{ color: "var(--primary-gold)" }}>
                    ✓
                  </span>
        ...
        {activeTab === "specifications" && (
          <div className="space-y-2 text-sm">
            {Object.entries(product.specifications).map(([key, value]) => (
              <div key={key} className="flex justify-between py-2.5 border-b" style={{ borderColor: "#f0f0f0" }}>
        ...
        {activeTab === "reviews" && (
          <div className="space-y-5">
            {product.reviews_data.map((review) => (
              <div key={review.id} className="pb-4 border-b text-sm" style={{ borderColor: "#f0f0f0" }}>
                  ...
                  <div className="flex gap-0.5">
                    {[...Array(5)].map((_, i) => (
                      <Star
                        ...
                        style={{ color: i < review.rating ? "var(--primary-gold)" : "#e5e5e5" }}
                      />
                    ))}
                  </div>
                </div>
                ...
                <p style={{ color: "#666", lineHeight: "1.5" }}>{review.comment}</p>
              </div>
            ))}
          </div>
        )}
